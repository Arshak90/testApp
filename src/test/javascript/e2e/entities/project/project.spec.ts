...
describe('Project e2e test', () => {
  ...
  it('should load Projects', async () => {
    ...
    expect(await projectComponentsPage.getTitle()).to.eq('testApp.project.home.title');
  });
  ...
  it('should load create Project page', async () => {
    ...
    expect(await projectUpdatePage.getPageTitle()).to.eq('testApp.project.home.createOrEditLabel');
  ...
  it('should create and save Projects', async () => {
    ...
    await projectComponentsPage.clickOnCreateButton();
    await promise.all([projectUpdatePage.setTitleInput('title'), projectUpdatePage.setCodeInput('code')]);
    expect(await projectUpdatePage.getTitleInput()).to.eq('title', 'Expected Title value to be equals to title');
    expect(await projectUpdatePage.getCodeInput()).to.eq('code', 'Expected Code value to be equals to code');
  ...
  it('should delete last Project', async () => {
    ...
    expect(await projectDeleteDialog.getDialogTitle()).to.eq('testApp.project.delete.question');
